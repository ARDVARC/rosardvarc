...
def _create_setpoint_timer_callback(event=None):
    ...
    if case == mission_state.FIND_RGV_1:
        x_set, y_set, z_set = _calc_orbit_setpoint_find(mission_state, RGV_state, current_UAS_pose)
    
    elif case == mission_state.FIND_RGV_2:
        x_set, y_set, z_set = _calc_orbit_setpoint_find(mission_state, RGV_state, current_UAS_pose)

    elif case == mission_state.TRACK_RGV_1: 
        x_set, y_set, z_set = _calc_orbit_setpoint_track(mission_state, RGV_state, current_UAS_pose)

    elif case == mission_state.TRACK_RGV_2:
        x_set, y_set, z_set = _calc_orbit_setpoint_track(mission_state, RGV_state, current_UAS_pose)

    elif case == mission_state.LOCALIZE_RGV_1:
        x_set, y_set, z_set = _calc_orbit_setpoint_localize(mission_state, RGV_state, current_UAS_pose)

    elif case == mission_state.LOCALIZE_RGV_2:
        x_set, y_set, z_set = _calc_orbit_setpoint_localize(mission_state, RGV_state, current_UAS_pose)
    
    elif case == mission_state.JOINT_LOCALIZE:
        x_set, y_set, z_set = _calc_orbit_setpoint_joint(mission_state, RGV_state, current_UAS_pose)
...
def setup():
    ...
    _create_setpoint_timer = rospy.Timer(rospy.Duration(CREATE_SETPOINT_RATE), _create_setpoint_timer_callback)
    _publish_setpoint_timer = rospy.Timer(rospy.Duration(PUBLISH_SETPOINT_RATE), _publish_setpoint_timer_callback)


def _calc_orbit_setpoint_find(mission_state: MissionState, RGV: EstimatedRgvState, UAS: PoseStamped) -> list:
    setpoint =  [CENTER_SETPOINT[0], CENTER_SETPOINT[1], UAS_ALTITUDE_SETPOINT]
    return setpoint

def _calc_orbit_setpoint_joint(mission_state: MissionState, RGV: EstimatedRgvState, UAS: PoseStamped) -> list:
...
def _calc_orbit_setpoint_track(mission_state: MissionState, RGV: EstimatedRgvState, UAS: PoseStamped) -> list:
...
def _calc_orbit_setpoint_localize(mission_state: MissionState, RGV: EstimatedRgvState, UAS: PoseStamped) -> list:
